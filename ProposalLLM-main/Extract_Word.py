...
# Get file name function - 修复文件名生成逻辑
def get_file_name(version, heading_text):
    # 根据标题级别生成正确的文件名
    if version[0] > 0 and version[1] == 0 and version[2] == 0:
        # 一级标题
        return f"{version[0]}- {heading_text}"
    elif version[0] > 0 and version[1] > 0 and version[2] == 0:
        # 二级标题
        return f"{version[0]}.{version[1]}- {heading_text}"
    elif version[0] > 0 and version[1] > 0 and version[2] > 0:
        # 三级标题
        return f"{version[0]}.{version[1]}.{version[2]}- {heading_text}"
    else:
        # 默认情况
        while version[-1] == 0:
            version.pop()
...
# Save content as new Word document and txt file
def save_content_to_new_doc(content, version, heading_text):
    from docx.oxml import OxmlElement
    from docx.oxml.ns import qn
    file_name = get_file_name(version, heading_text)
    ...
    for item in content:
        if isinstance(item, str):
            ...
            for row in table_data:
                txt_content.append(' | '.join(str(cell) for cell in row))
            ...
            # 添加图片描述到txt文件
            txt_content.append(f"[图片: {image_name}]")
            ...
            txt_content.append(item[1])

    # 保存docx文件
    new_doc.save(docx_file_name)
    
    # 保存txt文件
    with open(txt_file_name, 'w', encoding='utf-8') as f:
        f.write('\n'.join(txt_content))
    
    print(f"已保存: {docx_file_name} 和 {txt_file_name}")
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            ...
            if para.style.name.startswith('Heading'):
                # 如果遇到新标题，先保存之前的内容
                if current_level is not None and content_between_headings:
                ...
                print(f"处理标题: {current_heading_text} (级别: {heading_level})")
                #不加入标题进入正文
